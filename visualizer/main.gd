...
export var game_time_offset: float = 0
...
func _ready():
	...
	for netid in records:
		...
		if 'positions' in obj:
			...
			print('min velocity for ', netid, ' = ', min_vel)
			...
			for waypoint in obj['positions']:
				...
				if time >= game_time:
					if prev_time != null and prev_pos != null:
						...
						#draw_line(to_screen(lerped_pos + Vector2.DOWN * radius + Vector2.RIGHT * 100), to_screen(lerped_pos + Vector2.DOWN * radius + Vector2.LEFT * 100), Color.darkgreen)
						
						#var dir1 := Vector2(waypoint[1][Z + 1], waypoint[1][Z + 2])
						#var dir2 := Vector2(waypoint[1][Z + 3], waypoint[1][Z + 4])
						#draw_line(to_screen(lerped_pos), to_screen(lerped_pos + dir1 * 100), Color.darkgreen)
						#draw_line(to_screen(lerped_pos), to_screen(lerped_pos + dir2 * 100), Color.darkgreen)
...
export var zoom := 8.0
export var pad := Vector2(-480, 379) # -410 309
